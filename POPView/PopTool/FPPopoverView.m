...
- (void)drawRect:(CGRect)rect
{
    ...
    //content fill
    CGPathRef contentPath = [self createContentPathWithBorderWidth:0 arrowDirection:_arrowDirection];
    ...
    if(_arrowDirection == FPPopoverArrowDirectionUp)
    {
        ...
        end = CGPointMake(self.bounds.size.width/2.0,0);
    }
    else 
    {
        start = CGPointMake(self.bounds.size.width/2.0, 0);
...
//        CGContextSetRGBFillColor(ctx, 0.1, 0.1, 0.1, 1.0);
    ...
    //external border
    CGPathRef externalBorderPath = [self createContentPathWithBorderWidth:0.0 arrowDirection:_arrowDirection];
    CGContextBeginPath(ctx);
...
-(void)setupViews
{
    ...
    if(_arrowDirection == FPPopoverArrowDirectionUp)
    {
        contentRect.origin = CGPointMake(0, 12);
        contentRect.size = CGSizeMake(self.bounds.size.width, self.bounds.size.height);
    }
